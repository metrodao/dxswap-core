...
describe('DynamicFees', () => {
  ...
  async function calcOutput(token0In: BigNumber, token1In: BigNumber) {
    ...
    const token0Out = token0Reserve
      .sub(kReserve.div(token1Reserve.add(token1In.mul(SWAP_DEN.sub(swapFee)).div(SWAP_DEN))))
      .sub(bigNumberify(1))
    const token1Out = token1Reserve
      .sub(kReserve.div(token0Reserve.add(token0In.mul(SWAP_DEN.sub(swapFee)).div(SWAP_DEN))))
      .sub(bigNumberify(1))
